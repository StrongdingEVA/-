...
<script src="/Home/js/pinterest_grid.js"></script>
					...
					<div class="blog-posts">
						...
						<div class="last-article">
							...
							@elseif($articleInfo['article_video'])
								<video src="{{$articleInfo['article_video']}}" style="width:100%"></video>
							@endif
							<br>
								...
								<div class="response-item">
									<div class="response-item-head">
										<a href="javascript:void(0)">
											<img class="media-object" src="/upload/userimg/default.png" alt="">
										</a>
									</div>
									<div class="response-item-info">
										...
										<div class="info-username">
											<span>啊撒是的就看见啊收到了副科级阿里快速的减肥啊独立思考房间阿鲁卡多精神分裂看见阿里快速的减肥</span>
										</div>
										<div class="info-username last">
											<span class="time">2018-04-50</span>
											<span class="comment"><i class="reply"></i>回复</span>
										</div>
									</div>
								</div>

								<div class="response-item child">
									<div class="response-item-head">
										<a href="javascript:void(0)">
											<img class="media-object" src="/upload/userimg/default.png" alt="">
										</a>
	...
	<script>
		$(function() {
            ...
            var ueAns = UE.getEditor('ansFormUe',{initialFrameHeight:90,toolbarTopOffset:90,wordCount:false,elementPathEnabled:false});
			$(document).on('click','.createedui',function(){
				...
				if(thisClass == "edui-default"){
                    ...
                    ueAns.articleid = $(this).attr('articleid');
                    ueAns.commentid = $(this).attr('commentid');
                    ueAns.touserid = $(this).attr('this-user');
				    $(this).parents('ul').append($('#ansForm').show());
				}
			...
			$(".btn-primary").click(function(){
				if(!UE.getEditor('ansFormUe').getContent()){
					alert("内容不能为空");return;
				}
				...
				formData += '<input type="hidden" name="article_comment" value="'+ UE.getEditor('ansFormUe').getContent() +'">';
				formData += '</form></div>';
				//$(formData).submit(); //火狐浏览器不支持这种方法  改为下面
				$("body").append(formData);
				//ajax 方法
				var formDataSerialize = $("#tempForm").serialize();console.log(formDataSerialize);
