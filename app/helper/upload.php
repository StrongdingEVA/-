...
class Upload
{
	...
	//需要检测木马的文件类型
	private $checkType = array('jpg','gif','png','mp3','mp4');
    ...
    public function setDir($dir)
    {
    	if($dir != '')
    	{
    		if(is_array($dir))
    		{
    			...
    			{
    				if(is_array($value))			//图片、音视频为多数组的时候，获得多个地址
    				{
    					foreach($value as $k => $v)
    					{
		    				...
		    				$dir       = strtr($v,'\\','/');
		    				$d[$key][$k] = substr($dir,0,-1)=='/' ? $dir : $dir.'/';
    					}
    				} 
    				else 
    				{
	    				...
	    				$dir       = strtr($value,'\\','/');
	    				$d[$key]['0'] = substr($dir,0,-1)=='/' ? $dir : $dir.'/';
    				}
    			}
    			$this->dir = $d;
    		}
    ...
    public function execute()
    {
        ...
        {	
            ...
            { 	
                $keys = array_keys($_FILES[$field]['name']);		//0  1 数组
                $ks = array_keys($this->dir[$field]);				//保存地方，数组
                ...
                foreach($keys as $key)
                {
                	...
                	else
                	{
		                ...
		                else
		                {
		                    ...
		                    {
				                    //修改图片状态值
				                    $fileInfo[$key]['name']    = date("YmdHis",time()).mt_rand(100,999).'.'.$fileext;
				                    if(is_array($this->dir))
					    			{
					    				$dir     = $this->dir[$field][$key];  
					    			}
				                    else 
					    			{
					    				$dir     = $this->dir;  
				                    ...
				                    if($this->isForge == false)
				                    {
					                    ...
					                    {
					                    		IFile::mkdir($dir);
					                    		move_uploaded_file($_FILES[$field]['tmp_name'][$key],$dir.$fileInfo[$key]['name']);
					                    }
				                    }
				                    else
				                    {
				                    	IFile::xcopy($_FILES[$field]['tmp_name'][$key],$dir.$fileInfo[$key]['name']);
				                    }
		                    }
		                }
                	}
            ...
            else
            {
            	...
            	else
            	{
                	...
                	else
                	{
		                ...
		                {
			                //修改图片状态值
			                $fileInfo[0]['name']    = date("YmdHis",time()).mt_rand(100,999).'.'.$fileext;
			    			if(is_array($this->dir))
			    			{
			    				$dir     = $this->dir[$field]['0'];  
			    			}
			    			else 
			    			{
			    				$dir     = $this->dir;  
                            ...
                            if(in_array($fileext,array("jpg","png","jpeg","gif"))){
			    			    $dir .= "source/".date("Ymd",time())."/";
                            }else if(in_array($fileext,array("mp4","avi","mkv","flv"))){
                                $dir .= "video/".date("Ymd",time())."/";
                            }else{
                                $fileInfo[0]['flag'] = -9;
                                return false;
                            }
							...
							if($this->isForge == false)
							{
			                    ...
			                    {
			                    	IFile::mkdir($dir);
			                    	move_uploaded_file($_FILES[$field]['tmp_name'],$dir.$fileInfo[0]['name']);
			                    }
							}
							else
							{
								IFile::xcopy($_FILES[$field]['tmp_name'],$dir.$fileInfo[0]['name']);
							}
		                }
                	}
            	}
